...
public class ApiCartController {
    ...
    @DeleteMapping("/delete-ticket/{seatNumber}")
    public HttpStatus deleteCartItem(@PathVariable(value = "seatNumber") String seatNumber,HttpSession session){
        Map<String, CompanyCart> cart = (Map<String, CompanyCart>) session.getAttribute("company-cart");
        if (cart.containsKey(seatNumber)){
            cart.remove(seatNumber);
            session.setAttribute("company-cart", cart);
